...
class _CompleteProfilePageState extends State<CompleteProfilePage> {
  ...
  Future<void> _fetchCities(String provinceCode) async {
    setState(() { _cities = []; _barangays = []; _selectedCityName = null; _selectedBarangay = null; });
    try {
      final response = await http.get(Uri.parse('https://psgc.gitlab.io/api/provinces/$provinceCode/cities-municipalities/'));
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> _fetchBarangays(String cityCode) async {
    setState(() { _barangays = []; _selectedBarangay = null; });
    try {
      final response = await http.get(Uri.parse('https://psgc.gitlab.io/api/cities-municipalities/$cityCode/barangays/'));
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> _saveProfile() async {
    ...
    if (!_formKey.currentState!.validate() || _selectedBarangay == null || _selectedProvinceName == null || _selectedCityName == null) {
    ...
    try {
      ...
      if (locationData != null && locationData.longitude != null && locationData.latitude != null) {
        body['location'] = {
          'type': 'Point',
          'coordinates': [locationData.longitude!, locationData.latitude!], // [lng, lat]
        };
      }
      ...
      if (response.statusCode == 200) {
        ...
        await prefs.setString('municipality', updatedUser['municipality'] ?? '');
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Duplicate data!")));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.white, elevation: 0, title: const Text("Complete Your Profile", style: TextStyle(color: Colors.black))),
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<dynamic>(
                ...
                items: _cities.map((c) => DropdownMenuItem(value: c['code'], child: Text(c['name']))).toList(),
                onChanged: (v) {
                  final city = _cities.firstWhere((element) => element['code'] == v);
                  setState(() {
                    _selectedCityCode = v;
              ...
              DropdownButtonFormField<String>(
                ...
                items: _barangays.map((b) => DropdownMenuItem(value: b['name'] as String, child: Text(b['name']))).toList(),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isSubmitting ? const CircularProgressIndicator(color: Colors.white) : const Text("COMPLETE REGISTRATION", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
  ...
  // --- REUSABLE UI ---
  Widget _buildLabel(String text) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(text, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.black54)));
  InputDecoration _inputDecoration(String hint) => InputDecoration(
    hintText: hint, filled: true, fillColor: Colors.grey[50], contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
